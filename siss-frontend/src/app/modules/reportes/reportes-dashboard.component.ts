      ...
      <div class="w-full">
        ...
        <div class="mb-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          <div class="bg-white p-2 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap items-center gap-2">
            ...
            <a *ngIf="canSee('gestion_reportes:gestionar')" routerLink="gestion" class="flex items-center gap-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 px-4 py-2.5 rounded-xl font-bold transition-all ml-2">
        ...
        <div class="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-6 mb-10">
          <div *ngIf="kpis()?.consultasHoy != null" class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-blue-200 transition-all group relative overflow-hidden">
          ...
          <div *ngIf="kpis()?.pacientesNuevos != null" class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-purple-200 transition-all group relative overflow-hidden">
          ...
          <div *ngIf="kpis()?.stockCritico != null" class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-rose-200 transition-all group relative overflow-hidden">
          ...
          <div *ngIf="kpis()?.citasPendientes != null" class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-blue-200 transition-all group relative overflow-hidden">
            <div class="absolute -right-4 -bottom-4 w-24 h-24 bg-blue-50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
               <div class="p-2">
                 <ng-container *ngFor="let rep of reportesPorCategoria()[cat]">
                   <button (click)="ejecutarReporte(rep)"
...
export class ReportesComponent implements OnInit {
  ...
  cargarReportesDisponibles() {
    this.reportesSvc.obtenerMisReportes().subscribe(reportes => {
  ...
  canSee(slug: string): boolean {
    ...
    // Administrar catálogo de reportes exige rol ADMIN o permiso explícito reportes:gestionar
    if (slug === 'reportes:gestionar') {
      return this.authSvc.tieneRol('ADMIN') || this.authSvc.tienePermiso('reportes:gestionar');
    }

    if (this.authSvc.tienePermiso(slug)) return true;
    const [mod] = slug.split(':');
    if (mod === 'reportes') {
      return (
        this.authSvc.tieneAccesoModulo('reportes') ||
        this.authSvc.tienePermiso('reportes:generar') ||
        this.authSvc.tienePermiso('reportes:leer')
      );
    }
    if (this.authSvc.tieneAccesoModulo(mod)) return true;
    return false;
  }
